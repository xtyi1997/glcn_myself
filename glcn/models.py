...
class SGLCN(object):
    ...
    def _loss(self):
        ...
        # Graph Learning loss  生成一维矩阵ones
    ...
    def build(self):
        # 首先搞懂，这个h和S分别代表什么？ 这个inputs理解着是feature
        self.x, self.S = self.layers0(self.inputs)

        # self.glcn_adj=tf.Variable((13264,2),'glcn_adj')

        x1 = self.layers1(self.inputs, self.S)
        self.outputs = self.layers2(x1, self.S)

        # Store model variables for easy access   把变量提取出来
