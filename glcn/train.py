...
flags.DEFINE_string('dataset', 'pubmed', 'Dataset string.')  # 'cora', 'citeseer'
...
# Load data
adj, features, y_train, y_val, y_test, train_mask, val_mask, test_mask=load_data_my(FLAGS.dataset)
...
for epoch in range(FLAGS.epochs):
    ...
    outs = sess.run([model.opt_op, model.loss, model.accuracy,model.S], feed_dict=feed_dict)
    # Validation
    ...
    if cost < best:
        ...
        patience += 1
    # b = tf.sparse_tensor_to_dense(model.S, default_value=0, name='xxx')
    # op1=tf.assign(model.glcn_adj, model.S.indices)
    # sess.run(op1)
    # b=model.S.indices
    # adj=b
    # glcn_adj={'adj':b}
    # saver = tf.train.Saver([model.glcn_adj])
    # saver.save(sess, "./checkpoint_dir/adj.ckpt")
...
# saver.save(sess,"./checkpoint_dir/MyModel.ckpt")
